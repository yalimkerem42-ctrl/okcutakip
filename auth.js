...
async function handlePhotoUpload(file) {
  ...
  try {
    const storageRef = ref(storage, "users/" + currentUser.uid + "/profile_photo");
    ...
    console.error("[auth] Fotoğraf yüklenemedi:", err);
    if (typeof window.showFbError === "function") window.showFbError("Fotoğraf yüklenemedi. İnternet bağlantınızı kontrol edin.");
  } finally {
...
async function profileRemovePhoto() {
  if (!currentUser) return;
  try {
    try { await deleteObject(ref(storage, "users/" + currentUser.uid + "/profile_photo")); }
    catch (e) { /* dosya yoksa sorun değil */ }
    ...
    if (typeof window.showFbError === "function") window.showFbError("Fotoğraf kaldırılamadı. Tekrar deneyin.");
  }
}
